...
public struct PostNotification: ExpressionMacro {
    public static func expansion<Node: FreestandingMacroExpansionSyntax,
        ...
        guard let name = node.arguments.first(where: { $0.label == nil })?.expression else {
            throw MacroDiagnostics.errorMacroUsage(message: "Must specify a Notification name")
        }
        let object = node.arguments.first(where: { $0.label?.text == "object" })?.expression
        let userInfo = node.arguments.first(where: { $0.label?.text == "userInfo" })?.expression
        let notificationCenter = node.arguments.first(where: { $0.label?.text == "from" })?.expression
        let expr: ExprSyntax =
        """
