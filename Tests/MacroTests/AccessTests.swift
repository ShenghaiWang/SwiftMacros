...
final class AccessTests: XCTestCase {
    ...
    func testAccessNSCacheMacro() {
        assertMacroExpansion(
            ...
            var isPaidUser1 = NSObject() {
                get {
                  (cache.object(forKey: "AccessKey_isPaidUser1" as NSString) as? NSObject) ?? NSObject()
                }
                set {
                  cache.setObject(newValue, forKey: "AccessKey_isPaidUser1 as NSString")
                }
            }
            var isPaidUser2: NSObject? {
                get {
                  (cache.object(forKey: "AccessKey_isPaidUser2" as NSString) as? NSObject)
                }
                set {
                  if let value = newValue {
                    cache.setObject(value, forKey: "AccessKey_isPaidUser2 as NSString")
                  } else {
                    cache.removeObject(forKey: "AccessKey_isPaidUser2 as NSString")
                  }
                }
            }
            """,
